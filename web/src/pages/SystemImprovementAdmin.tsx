...
import { 
  ...
  Calendar, User, Send, ChevronLeft, ChevronRight,
  FileText, Download
} from 'lucide-react';
...
const SystemImprovementAdmin: React.FC = () => {
  ...
  const loadRequests = async () => {
    try {
      ...
      const response = await fetch(`/api/improvement/admin/system-requests?${params.toString()}`);
  ...
  const handleStatusChange = async (requestId: number, newStatus: string) => {
    try {
      const response = await fetch(`/api/improvement/system-requests/${requestId}/status`, {
        method: 'PUT',
        headers: {
  ...
  const handleSubmitResponse = async () => {
    ...
    try {
      const response = await fetch(`/api/improvement/system-requests/${selectedRequest.id}/responses`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <div className="app">
      ...
      />
      <main className="app-main">
        <div className="system-improvement-admin-container" style={{ width: '90%', margin: '0 auto' }}>
      ...
      <div className="requests-list">
        {requests.length === 0 ? (
          <div className="empty-state">
            ...
            <p>현재 필터 조건에 맞는 시스템 개선요청이 없습니다.</p>
          </div>
        ) : (
...
export default SystemImprovementAdmin;
