...
import { Send, Upload, X, MessageCircle, CheckCircle } from 'lucide-react';
...
const ESMRequestRegistration: React.FC = () => {
  ...
  // 첨부파일 기능 비활성화 플래그 (보안절차 통과 전 임시 비활성화)
  const ENABLE_ATTACHMENT_UPLOAD = false;
  ...
  // URL 파라미터에서 제목과 내용 읽어서 자동 채우기
  useEffect(() => {
    const titleParam = searchParams.get('title');
    const contentParam = searchParams.get('content');
    
    if (titleParam) {
      setRequestTitle(decodeURIComponent(titleParam));
    }
    
    if (contentParam) {
      setRequestContent(decodeURIComponent(contentParam));
    }
    
    // URL 파라미터가 있으면 작성 모드를 'manual'로 설정 (AI 자동작성이 아닌 수동 입력으로 간주)
    ...
    try {
      ...
      const htmlContent = `
        <div style="font-family: 'Malgun Gothic', '맑은 고딕', sans-serif; padding: 20px; max-width: 800px;">
          <h1 style="text-align: center; color: #333; margin-bottom: 30px;">ESM Request Document</h1>
  ...
  const handleSubmit = async () => {
    ...
    if (selectedTemplate) {
      ...
      try {
        ...
        // PDF 파일 객체 생성
        const pdfFile = new File([pdfBlob], `ESM_Request_${Date.now()}.pdf`, { type: 'application/pdf' });
  ...
  const submitRequest = async (finalAttachments: File[], pdfAttached: boolean = false) => {
    ...
    try {
      ...
      // 환경 설정 (기본값: dev)
      formDataToSubmit.append('environment', 'dev');
      ...
      const response = await fetch('/api/esm/requests', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        const message = result.warning 
          ? `ESM 요청이 저장되었습니다.\n(SAP Sales Cloud 연동 실패: ${result.error})`
          : pdfAttached 
            ? '요청이 등록되었습니다.\nSAP Sales Cloud(ESM)에 Case가 생성되었습니다.\n[PDF 첨부됨]'
            : '요청이 등록되었습니다.\nSAP Sales Cloud(ESM)에 Case가 생성되었습니다.';
        setSuccessMessage(message);
        setShowSuccessModal(true);
        
        // 2초 후 메인화면으로 이동하면서 refresh
        setTimeout(() => {
      ...
      <main className="app-main">
        ...
        <div className="request-form">
          <div className="form-section">
            ...
            {/* 첨부파일 등록 UI (보안절차 통과 전 임시 비활성화) */}
            {ENABLE_ATTACHMENT_UPLOAD && (
              <div className="form-group">
                <label>첨부파일</label>
...
export default ESMRequestRegistration;
