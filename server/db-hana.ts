...
async function seedBusinessDomainHierarchy() {
  try {
    await query(`INSERT INTO EAR.business_domains (DOMAIN_CODE, DOMAIN_NAME, DESCRIPTION)
      SELECT 'SAP', 'SAP Domain', 'SAP 업무 모듈 체계' FROM DUMMY
      WHERE NOT EXISTS (SELECT 1 FROM EAR.business_domains WHERE DOMAIN_CODE = 'SAP')`);

    const modules = [
      { code: 'CM.1', name: '통합', order: 5 },
      { code: 'MM.1', name: '구매처/자재마스터', order: 10 },
      { code: 'MM.2', name: 'Sourcing/계약관리', order: 20 },
      { code: 'MM.3', name: '구매발주관리', order: 30 },
      { code: 'MM.4', name: '재고관리', order: 40 },
      { code: 'MM.5', name: '인보이스관리', order: 50 },
      { code: 'PP.1', name: '제조/자원 마스터', order: 60 },
      { code: 'PP.2', name: '수요·공급 계획', order: 70 },
      { code: 'PP.3', name: '생산오더 관리', order: 80 },
      { code: 'PP.4', name: '생산 실행', order: 90 },
      { code: 'PP.5', name: '정산/분석', order: 100 },
      { code: 'HR.1', name: '조직/인사마스터', order: 110 },
      { code: 'HR.2', name: '채용/온보딩', order: 120 },
      { code: 'HR.3', name: '근태/휴가', order: 130 },
      { code: 'HR.4', name: '급여/복리', order: 140 },
      { code: 'HR.5', name: '성과/육성', order: 150 },
      { code: 'SD.1', name: '고객/가격 마스터', order: 160 },
      { code: 'SD.2', name: '견적/주문 관리', order: 170 },
      { code: 'SD.3', name: '납품/출고/물류', order: 180 },
      { code: 'SD.4', name: '청구/정산', order: 190 },
      { code: 'SD.5', name: '수금/채권/신용', order: 200 },
      { code: 'FI.1', name: '일반회계', order: 210 },
      { code: 'FI.2', name: '결산관리', order: 220 },
      { code: 'FI.3', name: '채권관리', order: 230 },
      { code: 'FI.4', name: '채무관리', order: 240 },
      { code: 'FI.5', name: '자산관리', order: 250 },
      { code: 'FI.6', name: '세무관리', order: 260 },
      { code: 'FI.7', name: '연결결산', order: 270 },
      { code: 'CO.1', name: '운영예산(Fund Center)', order: 280 },
      { code: 'CO.2', name: 'PS(WBS 투자예산)', order: 290 },
      { code: 'CO.3', name: 'CA(Cost Center 비용관리)', order: 300 },
      { code: 'CO.4', name: '제조원가/재고수불', order: 310 },
      { code: 'CO.5', name: 'PCA(Profit Center 성과관리)', order: 320 },
      { code: 'CO.6', name: 'PA(수익성분석)', order: 330 },
      { code: 'CO.7', name: '연결PA(수익성분석)', order: 340 },
      { code: 'BC.1', name: '계정/권한', order: 350 },
      { code: 'BC.2', name: '운영요청(티켓)', order: 360 },
      { code: 'BC.3', name: '모니터링/배치', order: 370 },
      { code: 'BC.4', name: '릴리즈(Transport)', order: 380 },
      { code: 'BC.5', name: '보안/복구', order: 390 }
    ];
    ...
    const level2Rows = [
      ...
      ['MM.1', 'MM.1.1', '자재마스터', 30],
      ['MM.1', 'MM.1.2', '구매처마스터', 40],
      ['MM.1', 'MM.1.3', '구매처 평가', 50],
      ['MM.2', 'MM.2.1', '구매 Sourcing 지정', 60],
      ['MM.2', 'MM.2.2', '계약관리', 70],
      ['MM.3', 'MM.3.1', '구매계획관리', 80],
      ['MM.3', 'MM.3.2', '구매발주', 90],
      ['MM.3', 'MM.3.3', '서비스구매', 100],
      ['MM.3', 'MM.3.4', '구매요청 관리', 110],
      ['MM.3', 'MM.3.5', '구매 분석', 120],
      ['MM.3', 'MM.3.6', '발주 협업', 130],
      ['MM.4', 'MM.4.1', '자재 입고', 140],
      ['MM.4', 'MM.4.2', '자재 출고', 150],
      ['MM.4', 'MM.4.3', '창고 관리', 160],
      ['MM.4', 'MM.4.4', '출하 관리', 170],
      ['MM.5', 'MM.5.1', '인보이스 관리', 180],
      ['MM.5', 'MM.5.2', '매입채무 관리', 190],
      ['MM.5', 'MM.5.3', '3-way match 예외', 200],
      ['MM.5', 'MM.5.4', '인보이스 협업관리', 210],
      ['PP.1', 'PP.1.1', '제품/자재마스터', 220],
      ['PP.1', 'PP.1.2', 'BOM 관리', 230],
      ['PP.1', 'PP.1.3', 'Routing/Workcenter/Production Version', 240],
      ['PP.2', 'PP.2.1', '수요예측/계획', 250],
      ['PP.2', 'PP.2.2', 'S&OP', 260],
      ['PP.2', 'PP.2.3', 'MPS', 270],
      ['PP.3', 'PP.3.1', 'MRP 실행/예외', 280],
      ['PP.3', 'PP.3.2', '오더 생성/릴리즈', 290],
      ['PP.3', 'PP.3.3', '자재가용성/대체', 300],
      ['PP.4', 'PP.4.1', '생산 투입(GI)', 310],
      ['PP.4', 'PP.4.2', '가공비 처리', 320],
      ['PP.4', 'PP.4.3', 'Scrap 처리', 330],
      ['PP.4', 'PP.4.4', 'WIP 관리', 340],
      ['PP.5', 'PP.5.1', '반/완제품 입고', 350],
      ['PP.5', 'PP.5.2', '간접비 배부', 360],
      ['PP.5', 'PP.5.3', '생산 오더 정산', 370],
      ['PP.5', 'PP.5.4', '생산 실적 레포트', 380],
      ['HR.1', 'HR.1.1', '조직/직무', 390],
      ['HR.1', 'HR.1.2', '인사기본', 400],
      ['HR.1', 'HR.1.3', '권한/접근', 410],
      ['HR.2', 'HR.2.1', '채용요청', 420],
      ['HR.2', 'HR.2.2', '지원/면접', 430],
      ['HR.2', 'HR.2.3', '오퍼/계약', 440],
      ['HR.2', 'HR.2.4', '온보딩/계정', 450],
      ['HR.3', 'HR.3.1', '근태관리', 460],
      ['HR.3', 'HR.3.2', '근태마감처리', 470],
      ['HR.3', 'HR.3.3', '스케줄/교대 관리', 480],
      ['HR.3', 'HR.3.4', '예외처리', 490],
      ['HR.4', 'HR.4.1', '급여데이터 관리', 500],
      ['HR.4', 'HR.4.2', '급여계산', 510],
      ['HR.4', 'HR.4.3', '지급/전표', 520],
      ['HR.4', 'HR.4.4', '연말정산', 530],
      ['HR.5', 'HR.5.1', '평가관리', 540],
      ['HR.5', 'HR.5.2', '목표/OKR', 550],
      ['HR.5', 'HR.5.3', '교육/역량', 560],
      ['HR.5', 'HR.5.4', '보상연계', 570],
      ['SD.1', 'SD.1.1', '고객마스터', 580],
      ['SD.1', 'SD.1.2', '가격/할인', 590],
      ['SD.1', 'SD.1.3', '신용한도/위험', 600],
      ['SD.2', 'SD.2.1', '견적/계약', 610],
      ['SD.2', 'SD.2.2', '판매오더', 620],
      ['SD.2', 'SD.2.3', 'ATP/가용성', 630],
      ['SD.2', 'SD.2.4', '변경/승인', 640],
      ['SD.2', 'SD.2.5', '프로모션 적용', 650],
      ['SD.3', 'SD.3.1', '납품생성', 660],
      ['SD.3', 'SD.3.2', '피킹/패킹', 670],
      ['SD.3', 'SD.3.3', '출고(PGI)', 680],
      ['SD.3', 'SD.3.4', '운송/배송추적', 690],
      ['SD.4', 'SD.4.1', '청구문서', 700],
      ['SD.4', 'SD.4.2', '세금/조건', 710],
      ['SD.4', 'SD.4.3', '청구차이/예외', 720],
      ['SD.4', 'SD.4.4', '정산/조정', 730],
      ['SD.4', 'SD.4.5', '반품/클레임', 740],
      ['SD.5', 'SD.5.1', '수금', 750],
      ['SD.5', 'SD.5.2', '대사/미결', 760],
      ['SD.5', 'SD.5.3', '연체/독촉', 770],
      ['SD.5', 'SD.5.4', '채권분석/대손', 780],
      ['FI.1', 'FI.1.1', '기준정보관리', 790],
      ['FI.1', 'FI.1.2', '전표관리', 800],
      ['FI.2', 'FI.2.1', '결산일정 관리', 810],
      ['FI.2', 'FI.2.2', '결산조정 관리', 820],
      ['FI.2', 'FI.2.3', '결산보고', 830],
      ['FI.2', 'FI.2.4', '연말결산(연결사)', 840],
      ['FI.3', 'FI.3.1', '기준정보관리', 850],
      ['FI.3', 'FI.3.2', '채권전표관리', 860],
      ['FI.3', 'FI.3.3', '채권관리', 870],
      ['FI.4', 'FI.4.1', '기준정보관리', 880],
      ['FI.4', 'FI.4.2', '채무전표관리', 890],
      ['FI.4', 'FI.4.3', '선급금/가지급금 관리', 900],
      ['FI.4', 'FI.4.4', '채무지급 및 반제', 910],
      ['FI.4', 'FI.4.5', '법인카드 정산', 920],
      ['FI.5', 'FI.5.1', '자산기준정보관리', 930],
      ['FI.5', 'FI.5.2', '자산취득관리', 940],
      ['FI.5', 'FI.5.3', '자산운용관리', 950],
      ['FI.5', 'FI.5.4', '자산처분관리', 960],
      ['FI.5', 'FI.5.5', '감가상각', 970],
      ['FI.6', 'FI.6.1', '기준정보관리', 980],
      ['FI.6', 'FI.6.2', '부가가치세관리', 990],
      ['FI.6', 'FI.6.3', '원천세 관리', 1000],
      ['FI.6', 'FI.6.4', '법인세 관리', 1010],
      ['FI.6', 'FI.6.5', '지방세 관리', 1020],
      ['FI.7', 'FI.7.1', '연결 기준정보 관리', 1030],
      ['FI.7', 'FI.7.2', '내부거래 관리', 1040],
      ['FI.7', 'FI.7.3', '연결 결산 조정', 1050],
      ['FI.7', 'FI.7.4', '연결재무 결산 마감', 1060],
      ['CO.1', 'CO.1.1', '운영예산 기준정보', 1070],
      ['CO.1', 'CO.1.2', '운영예산 예산관리', 1080],
      ['CO.1', 'CO.1.3', '운영예산 실적', 1090],
      ['CO.2', 'CO.2.1', '투자예산 기준정보', 1100],
      ['CO.2', 'CO.2.2', '투자예산 예산관리', 1110],
      ['CO.2', 'CO.2.3', '투자예산 실적', 1120],
      ['CO.3', 'CO.3.1', '비용관리 기준정보', 1130],
      ['CO.3', 'CO.3.2', '비용관리 결산/정산', 1140],
      ['CO.3', 'CO.3.3', '비용관리 실적', 1150],
      ['CO.4', 'CO.4.1', '제조원가 기준정보', 1160],
      ['CO.4', 'CO.4.2', '제조원가 결산/정산', 1170],
      ['CO.4', 'CO.4.3', '제조원가 실적', 1180],
      ['CO.5', 'CO.5.1', 'PCA 기준 정보', 1190],
      ['CO.5', 'CO.5.2', 'PCA 결산/정산', 1200],
      ['CO.5', 'CO.5.3', 'PCA 실적', 1210],
      ['CO.6', 'CO.6.1', 'PA 기준정보', 1220],
      ['CO.6', 'CO.6.2', 'PA 결산/정산', 1230],
      ['CO.6', 'CO.6.3', 'PA 실적', 1240],
      ['CO.7', 'CO.7.1', '연결PA 기준정보', 1250],
      ['CO.7', 'CO.7.2', '연결PA 결산/정산', 1260],
      ['CO.7', 'CO.7.3', '연결PA 실적', 1270],
      ['BC.1', 'BC.1.1', '사용자 생성', 1280],
      ['BC.1', 'BC.1.2', '역할/권한 부여', 1290],
      ['BC.1', 'BC.1.3', 'SoD 점검', 1300],
      ['BC.1', 'BC.1.4', '접근로그', 1310],
      ['BC.2', 'BC.2.1', '티켓접수', 1320],
      ['BC.2', 'BC.2.2', '자동분류/우선순위', 1330],
      ['BC.2', 'BC.2.3', '표준응답/가이드', 1340],
      ['BC.2', 'BC.2.4', 'SLA 관리', 1350],
      ['BC.3', 'BC.3.1', '배치잡', 1360],
      ['BC.3', 'BC.3.2', '성능 모니터링', 1370],
      ['BC.3', 'BC.3.3', '에러로그', 1380],
      ['BC.3', 'BC.3.4', '인터페이스 상태', 1390],
      ['BC.4', 'BC.4.1', 'Transport 승인', 1400],
      ['BC.4', 'BC.4.2', '릴리즈 노트/공지', 1410],
      ['BC.4', 'BC.4.3', '롤백계획', 1420],
      ['BC.5', 'BC.5.1', '백업/복구', 1430],
      ['BC.5', 'BC.5.2', '취약점 점검', 1440],
      ['BC.5', 'BC.5.3', '권한감사', 1450],
      ['BC.5', 'BC.5.4', '비상계정', 1460]
    ];
...
async function createDefaultPortalUsers() {
  try {
    const portalUsers = [
      { userid: 'portal-admin', fullName: '포털 관리자', isAdmin: true },
      { userid: 'portal-user', fullName: '포털 사용자', isAdmin: false }
    ];
    ...
    console.error('포털 기본 계정 설정 실패:', error.message);
  }
}
