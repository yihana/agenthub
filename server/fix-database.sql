-- 기존 테이블 삭제 및 재생성
DROP TABLE IF EXISTS ear_requests CASCADE;
DROP TABLE IF EXISTS ear_request_templates CASCADE;
DROP TABLE IF EXISTS ear_keywords CASCADE;

-- UNIQUE 제약조건이 포함된 테이블 재생성
...
-- 인덱스 생성
CREATE INDEX idx_ear_keywords_keyword ON ear_keywords(keyword);
CREATE INDEX idx_ear_keywords_category ON ear_keywords(category);
CREATE INDEX idx_ear_request_templates_keyword_id ON ear_request_templates(keyword_id);
CREATE INDEX idx_ear_requests_status ON ear_requests(status);
CREATE INDEX idx_ear_requests_created_at ON ear_requests(created_at);

-- 초기 키워드 데이터 삽입
INSERT INTO ear_keywords (keyword, display_name, category) VALUES
('방화', '방화벽 오픈 신청', '보안'),
('방화벽', '방화벽 오픈 신청', '보안'),
...
FROM ear_keywords k WHERE k.keyword = '방화';
...
FROM ear_keywords k WHERE k.keyword = 'firewall';
...
FROM ear_keywords k WHERE k.keyword = '시스템';
...
FROM ear_keywords k WHERE k.keyword = '계정';
...
FROM ear_keywords k WHERE k.keyword = '장비';
